...
async function runTaskExecute(task: TaskItem) {
  ...
  try {
    // Kill terminals on the same port across ALL workspaces
    if (task.url) {
      const newUrl = new URL(task.url);
      const newPort = newUrl.port || (newUrl.protocol === 'https:' ? '443' : '80');
      for (const [otherWs, tabs] of Object.entries(terminalTabs.value)) {
        const toKill = tabs.filter(t => {
          if (!t.url) return false;
          try {
            const o = new URL(t.url);
            const p = o.port || (o.protocol === 'https:' ? '443' : '80');
            return p === newPort;
          } catch { return false; }
        });
        for (const ot of toKill) {
          await invoke("terminal_kill", { terminalId: ot.id }).catch(() => {});
        }
        terminalTabs.value[otherWs] = tabs.filter(t => !toKill.includes(t));
        if (terminalTabs.value[otherWs].length === 0) delete terminalTabs.value[otherWs];
      }
    }
    ...
    >
      ...
      >
        ...
        >
          ...
          <svg width="32" height="32" viewBox="0 0 24 24" :fill="ws.color || '#0078d4'" xmlns="http://www.w3.org/2000/svg"><path d="M21.5 3h-19A1.504 1.504 0 0 0 1 4.5v15A1.5 1.5 0 0 0 2.5 21h19a1.5 1.5 0 0 0 1.5-1.5v-15A1.504 1.504 0 0 0 21.5 3zm.5 16.5a.501.501 0 0 1-.5.5h-19a.501.501 0 0 1-.5-.5v-15a.506.506 0 0 1 .5-.5h19a.506.506 0 0 1 .5.5zM5.354 15.354l-.707-.707L7.793 11.5 4.646 8.354l.707-.707L9.207 11.5zM15 15h-5v-1h5z"/></svg>
        </div>
      </div>
    </div>
